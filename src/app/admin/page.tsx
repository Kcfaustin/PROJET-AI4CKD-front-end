   ...
   useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const userResponse = await getCurrentUser() as ApiResponse<User>;
        ...
        try {
          const doctorsResponse = await getDoctors() as ApiResponse<Doctor[]>;
          
          // Validation de la réponse
          if (!doctorsResponse?.success) {
            throw new Error(doctorsResponse.message || "Format de réponse invalide");
          }
          
          const doctorsData = doctorsResponse.doctors || doctorsResponse.data || [];
          
          // Filtrage typé
          const doctorsList = doctorsData.filter((user: User): user is Doctor => 
            user.role === 'doctor'
          );
          
          // Enrichissement des données
          const enhancedDoctors = doctorsList.map((doctor: Doctor) => ({
            ...doctor,
            patients_count: Math.floor(Math.random() * 30) + 1,
            last_login: doctor.updated_at ? 
              new Date(doctor.updated_at).toISOString() : 
